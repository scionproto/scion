...
lint-go-golangci:
	$(info ==> $@)
	@tools/quiet bazel run --config=quiet @rules_go//go -- run github.com/golangci/golangci-lint/cmd/golangci-lint@v2.1.6 run --config="${PWD}/.golangci.yml" --timeout=3m $(GO_BUILD_TAGS_ARG),lint --exclude-dirs doc ./...
