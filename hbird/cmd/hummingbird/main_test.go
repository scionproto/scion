...
func TestBandwidthEncodingOverflow(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if err != nil {
			...
			_, err = ComputeAuthenticationKey(ResInfo{Bandwidth: Bandwidth{kbps: tc.bw}},
				([16]byte)(masterKey))
			if err == nil {
...
func checkResInfo(resInfo ResInfo, masterKey [16]byte) (*[16]byte, error) {
	return ComputeAuthenticationKey(resInfo, masterKey)
}
