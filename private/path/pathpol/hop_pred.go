...
func NewHopPredicate() *HopPredicate {
	return &HopPredicate{IfIDs: make([]iface.IfIDType, 1)}
}
...
func HopPredicateFromString(str string) (*HopPredicate, error) {
	...
	var ifIDs = make([]iface.IfIDType, 1)
	// Parse ISD
...
func parseIfID(str string) (iface.IfIDType, error) {
	ifID, err := strconv.ParseUint(str, 10, 64)
	if err != nil {
		return 0, err
	}
	return iface.IfIDType(ifID), nil
}
