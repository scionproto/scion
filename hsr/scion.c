...
static inline int needs_local_processing(SCIONCommonHeader *sch);
...
static inline int calc_fwding_ingress(SCIONCommonHeader *sch);
...
static inline void relay_cert_server_packet(struct rte_mbuf *m,
        uint8_t from_local_socket, uint8_t dpdk_rx_port);
static inline void process_path_mgmt_packet(struct rte_mbuf *m,
        uint8_t from_local_ad, uint8_t dpdk_rx_port);
...
// deliver packet to endhost in current AS
