...
import hashlib, time, random, operator
    ...
    def parse(self):
        ...
        for f in fs:
            if f.tag == "WantedAD":
                ...
                self.min_max[f.tag] = int(f.text)
        ps = policy.find("Properties")
        for p in ps:
            self.properties[p.tag] = int(p.text)
...
class PathStore(object):
    ...
    def get_paths(self, k=10):
        ...
        if len(ret) < k:
            ret += self.candidates
            ret = sorted(ret, key=lambda PathInfo: PathInfo.fidelity, reverse=True)
