...
func TestExtension(t *testing.T) {
	...
	for dscr, input := range tests {
		t.Run(dscr, func(t *testing.T) {
			hbh, e2e, err := ValidateExtensions(input.InputExtensions)
			input.Assertion(t, err)
			assert.Equal(t, hbh, input.ExpectedOutputHBH, "HBH")
			assert.Equal(t, e2e, input.ExpectedOutputE2E, "E2E")
		})
	}
}
