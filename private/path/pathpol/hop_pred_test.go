...
func TestNewHopPredicate(t *testing.T) {
	tests := map[string]struct {
		...
		"ISD wildcard": {
			In:    "0",
			HP:    &HopPredicate{ISD: 0, AS: 0, IfIDs: []ifid.IfIDType{0}},
			Valid: true,
		},
		"AS, IF wildcard omitted": {
			In:    "1",
			HP:    &HopPredicate{ISD: 1, AS: 0, IfIDs: []ifid.IfIDType{0}},
			Valid: true,
		},
		"IF wildcard omitted": {
			In:    "1-0",
		...
		"ISD wildcard, AS set": {
			In:    "0-1#0",
			HP:    &HopPredicate{ISD: 0, AS: 1, IfIDs: []ifid.IfIDType{0}},
			Valid: true,
		},
		"ISD wildcard, AS set, interface set": {
			In:    "0-1#2",
			HP:    &HopPredicate{ISD: 0, AS: 1, IfIDs: []ifid.IfIDType{2}},
			Valid: true,
		},
		...
		"IF wildcard omitted, AS set": {
			In:    "1-2",
			HP:    &HopPredicate{ISD: 1, AS: 2, IfIDs: []ifid.IfIDType{0}},
			Valid: true,
		},
		"two IfIDs": {
			In:    "1-2#3,4",
			HP:    &HopPredicate{ISD: 1, AS: 2, IfIDs: []ifid.IfIDType{3, 4}},
			Valid: true,
		},
		"three IfIDs": {
			In:    "1-2#3,4,5",
...
func TestJsonConversion(t *testing.T) {
	tests := map[string]struct {
		...
		"Normal predicate": {
			HP: &HopPredicate{ISD: 1, AS: 2, IfIDs: []ifid.IfIDType{1, 2}},
		},
		"wildcard predicate": {
			HP: &HopPredicate{ISD: 1, AS: 2, IfIDs: []ifid.IfIDType{0}},
		},
		"only ifIDs": {
			HP: &HopPredicate{IfIDs: []ifid.IfIDType{0}},
		},
	}
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
