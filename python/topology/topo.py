...
from python.topology.common import (ArgsBase, join_host_port, json_default, SCION_SERVICE_NAMES,
                                    TopoID)
...
class TopoGenArgs(ArgsBase):
    ...
    def __init__(self, args: ArgsBase, topo_config, subnet_gen4: SubnetGenerator,
                 subnet_gen6: SubnetGenerator, default_mtu: int):
        """
...
class TopoGenerator(object):
    ...
    def _register_br_entries(self, topo_id, as_conf):
        ...
        for (linkto, remote, attrs, l_br, r_br, l_ifid, r_ifid) in self.links[topo_id]:
            self._register_br_entry(topo_id, l_ifid, remote, r_ifid, linkto, attrs, l_br, r_br,
                                    addr_type)

    def _register_br_entry(self, local, l_ifid, remote, r_ifid, remote_type, attrs, local_br,
                           remote_br, addr_type):
    ...
    def _gen_br_entry(self, local, l_ifid, remote, r_ifid, remote_type, attrs, local_br, remote_br,
                      addr_type):
        link_addr_type = addr_type_from_underlay(attrs.get('underlay', DEFAULT_UNDERLAY))
    ...
    def _write_as_topo(self, topo_id, _as_conf):
        ...
        contents_json = json.dumps(self.topo_dicts[topo_id], default=json_default, indent=2)
    ...
    def _write_ifids(self):
        ...
        write_file(list_path, yaml.dump(self.ifid_map, default_flow_style=False))
