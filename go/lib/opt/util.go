...
func (e *Extn) DataHashed(packet interface{}) []byte {
	shaChecksum := sha256.Sum256(make(common.RawBytes, 16))
	hashed := shaChecksum[:]
	return hashed
}
...
func (e *Extn) InitializePVF(sessionKey []byte, payload []byte) {
	dataHash := e.DataHashed(payload)
        copy(e.PVF, cbcMAC(sessionKey, dataHash))
...
func (e *Extn) UpdatePVF(packet *interface{}) {
	// Missing Calc DRKey
	localSecret, _ := make(common.RawBytes, 16), common.NewError("") //packet.CalcDRKey()
	// K_{AS_i}^session = PRF_{AS_i -> S, D}(SessionId)
	sessionKey := cbcMAC(localSecret, e.SessionId)
	copy(e.PVF, cbcMAC(sessionKey, e.PVF))
}

func cbcMAC(session []byte, PVF []byte) []byte {
	// Missing util CBCMac
	mac, _ := make(common.RawBytes, 16), common.NewError("") // util.CBCMac(session, PVF)
	return mac
}