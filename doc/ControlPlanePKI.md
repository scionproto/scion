...
switch the latest on the `Validity.NotBefore` time, since SCION requires synchronized clocks.
...
- __OptionalDistributionPoints__: Array string. Additional optimistic certificate revocation
...
the `OptionalDistributionPoints` field in the AS cert to nominate distribution points that are
