...
Each end-to-end test is a standalone Go binary that targets a containerlab lab
generated by testgen. They discover the running lab from the generated
artifacts (`gen/<lab>.clab.yml` + `gen/sciond_addresses.json`), then drive the
SCION tooling **inside** the node containers via `docker exec` — so no host-side
SCION reachability is required. Each prints a condensed source×destination
result matrix. The tests deliberately do not share code.

- [`e2e_scion`](e2e_scion) — runs the `scion` CLI (`ping`, `showpaths`) between
  every pair of ASes.
- [`e2e_http`](e2e_http) — an HTTP/3-over-SCION test. The single binary has
  three subcommands: `server` and `client` (run inside the nodes), and `run`
  (the driver). `server`/`client` build QUIC on a SCION `snet` socket
