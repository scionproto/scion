...
func newTrc(isd addr.ISD, iconf *conf.Isd, path string) (*trc.TRC, error) {
	...
	for _, cia := range iconf.Trc.CoreIAs {
		...
		as.OnlineKey, err = getPrivKey(online, as.OnlineKeyAlg)
		if err != nil {
			return nil, err
		}
		...
		as.OfflineKey, err = getPrivKey(offline, as.OfflineKeyAlg)
		if err != nil {
			return nil, err
		}
		ases = append(ases, as)
...
func getPrivKey(asKey common.RawBytes, keyType string) (common.RawBytes, error) {
	switch keyType {
	case crypto.Ed25519:
		return common.RawBytes(ed25519.PrivateKey(asKey)), nil
	default:
	}
	return nil, common.NewBasicError("Unsupported key type", nil, "type", keyType)
}
