...
\noindent{\bf AD certificate: } The certificates of all participating \ADs to a \TD are issued by the certificate authority of \TD and stored at \TDC \ADs and the owner of the certificates. Certificate servers are responsible for managing a certificate database and providing certificates to requesting \ADs. An \AD certificate contains the public key of an \AD signed by the \TD's certificate authority. An \AD, using \AD certificates, verifies the signatures of other \ADs in inter-\AD communication (e.g., PCBs signed by its providers).
...
\item \ADs know some public keys of \TDC \ADs \soobum{necessary? This seems to be a vague assumption} \tenma{Should be. I think it
