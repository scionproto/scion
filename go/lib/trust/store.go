...
type Store struct {
	...
	maxTrcMap map[addr.ISDInt]uint64
...
func NewStore(certDir, cacheDir, eName string) (*Store, error) {
	s := &Store{certDir: certDir, cacheDir: cacheDir, eName: eName,
		...
		maxTrcMap:   make(map[addr.ISDInt]uint64)}
	s.initChains()
	s.initTRCs()
	return s, nil
}
...
func (s *Store) GetTRC(isd addr.ISDInt, ver uint64) *trc.TRC {
	s.trcLock.RLock()
...
func (s *Store) GetNewestTRC(isd addr.ISDInt) *trc.TRC {
	s.trcLock.RLock()
