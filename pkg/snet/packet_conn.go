...
func (c *SCIONPacketConn) lastHop(p *Packet) (*net.UDPAddr, error) {
	...
	case onehop.PathType:
		...
		return c.interfaceIDToAddr(ifID)
	case epic.PathType:
		var path epic.Path
		if err := path.DecodeFromBytes(rpath.Raw); err != nil {
			return nil, err
...
func (c *SCIONPacketConn) interfaceIDToAddr(ifID uint16) (*net.UDPAddr, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond)
	defer cancel()
	intfs, err := c.Topology.Interfaces(ctx)
	if err != nil {
		return nil, serrors.Wrap("resolving interfaces address (fetching interfaces)", err, "interface", ifID)
	}
