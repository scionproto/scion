...
from lib.drkey.opt.protocol import (get_sciond_params,
                                    ...
                                    generate_sessionID,
                                    generate_pvf, SCIONOPTKeyExchangeError)
...
class E2EClient(TestClientBase):
    ...
    def _build_pkt(self, path=None):
        ...
        try:  # check if we already have exchanged keys
            ...
            drkey, misc = self._exchange_keys(spkt, extn, dst_isd_as, path_meta)
        ...
        if misc.drkeys:
            ...
            pvfs = generate_intermediate_pvfs(spkt,
                                              (drkey.src_ia.int(), drkey.drkey),
                                              ias_keylist)
    ...
    def _exchange_keys(self, spkt, extn, dst_isd_as, path_meta):
        # send shared keys and meta data to destination
        ...
        data = [extn.sessionID, cryptor.encrypt(key_exchange_bytes)]
        # send keys
        ...
        while retries < max_retries:
            try:
                ...
                dst_addr = str(spkt.addrs.dst.host.addr)
                path_m_str = str(path_meta)
                self.sent_keys[(dst_isd_as, dst_addr, path_m_str, extn.sessionID)] = (drkey, misc)
                return drkey, misc
            except socket.timeout:
                retries += 1
                self._send_pkt(spkt)
        raise SCIONOPTKeyExchangeError("OPT Key exchange timed out after %s retries" % max_retries)
...
class E2EServer(TestServerBase):
    ...
    def _handle_request(self, spkt):
        ...
        if raw_pld.startswith(key_exch_tag):  # get keys from key exchange
            self._receive_keys(spkt, src_isd_as, key_exch_tag, raw_pld)
        else:
        ...
        if not extn:  # packet has no OPT extension, we are done
            return True
        ...
        # Received valid packet, send "pong".
        logging.debug('%s:%d: ping received, sending pong.',
                      self.addr.host, self.sock.port)

        # Generate response
        spkt = self._generate_response(src_ia, src_addr, src_port)
    ...
    def _receive_keys(self, spkt, src_isd_as, key_exch_tag, raw_pld):
        # decrypt encrypted payload
        sessionID = raw_pld[len(key_exch_tag):len(key_exch_tag) + 16]
        drkey, _ = _try_sciond_api_src(spkt, sessionID, self._connector)
        safe_key = base64.urlsafe_b64encode(drkey.drkey + drkey.drkey)
        cryptor = encryption.Fernet(safe_key)
        misc_keys = cryptor.decrypt(raw_pld[len(key_exch_tag) + 16:])
    ...
    def _generate_response(self, src_ia, src_addr, src_port):
        ...
        if misc.drkeys:
            ...
            OVs = opvs
        new_extn.OVs = OVs
        logging.debug("Raw packet header being sent: {}".format(binascii.hexlify(new_extn.pack())))
        return spkt
...
def _try_sciond_api_src(spkt, sessionID, connector):
    start = time.time()
    while time.time() - start < API_TOUT:
        try:
            request_parameters = get_sciond_params_src(spkt, sessionID)
            drkey, misc = lib_sciond.get_protocol_drkey(
                request_parameters,
                connector=connector)
        except lib_sciond.SCIONDConnectionError as e:
