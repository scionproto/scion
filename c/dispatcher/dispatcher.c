...
void send_scmp_general_reply(uint8_t *buf, SCMPL4Header *scmpptr, HostAddr *from, uint16_t type);
void deliver_scmp_general_reply(uint8_t *buf, SCMPL4Header *scmp, int len, HostAddr *from);
...
void process_scmp(uint8_t *buf, SCMPL4Header *scmp, int len, HostAddr *from)
{
    ...
    if (ntohs(scmp->class_) == SCMP_GENERAL_CLASS)  {
        ...
        case SCMP_ECHO_REQUEST:
            send_scmp_general_reply(buf, scmp, from, SCMP_ECHO_REPLY);
            return;
        case SCMP_TRACEROUTE_REQUEST:
            send_scmp_general_reply(buf, scmp, from, SCMP_TRACEROUTE_REPLY);
            return;
        case SCMP_RECORDPATH_REQUEST:
            send_scmp_general_reply(buf, scmp, from, SCMP_RECORDPATH_REPLY);
            return;
        case SCMP_ECHO_REPLY:
        case SCMP_TRACEROUTE_REPLY:
        case SCMP_RECORDPATH_REPLY:
            deliver_scmp_general_reply(buf, scmp, len, from);
            return;
        }
    }
...
void send_scmp_general_reply(uint8_t *buf, SCMPL4Header *scmp, HostAddr *from, uint16_t type)
{
    SCIONCommonHeader *sch = (SCIONCommonHeader *)buf;
    reverse_packet(buf);
    scmp->type = htons(type);
    update_scmp_checksum(buf);
    zlog_debug(zc, "send SCMP reply to %s:%d", addr_to_str(from->addr, from->addr_type, NULL),
            ntohs(from->port));
...
void deliver_scmp_general_reply(uint8_t *buf, SCMPL4Header *scmp, int len, HostAddr *from)
{
    ...
    uint64_t id = ((uint64_t *)pld->info)[0];

    PingEntry *e;
    HASH_FIND(hh, ping_list, &id, sizeof(id), e);
    if (e != NULL) {
        zlog_debug(zc, "SCMP reply (%ld) entry found", be64toh(id));
        deliver_data(e->sock, from, buf, len);
    } else {
        zlog_warn(zc, "SCMP reply (%lu) entry not found", be64toh(id));
    }
    free(pld);
}
