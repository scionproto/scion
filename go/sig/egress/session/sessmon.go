...
func (sm *sessMonitor) updateRemote() {
	...
	if since > tout {
		...
		} else if currSessPath.PathEntry().Path.Expiry().Before(time.Now().Add(safetyInterval)) {
			// TTL is about to expire soon.
			if p.PathEntry().Path.Expiry().Before(time.Now().Add(safetyInterval)) {
				// Updated path is about to expire also, so let's switch to a different path.
				currSessPath = sm.getNewPath(p)
				sm.needUpdate = true
				isHealthy = false
			} else {
