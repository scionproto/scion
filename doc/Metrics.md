...
func newOriginator() originator {
    ...
    // Initialize default values for the labels that will be used
    // to initial register the metrics so as to avoid hidden metrics.
    // Preferably, a value that will create a timestamp in prometheus,
    // that will be later be used
    labels := OriginatorLabels{Result: promSuccess, EfIfID: "Nan"}

    return originator{
        beacons: prom.NewCounterVecWithLabels(Namespace, sub, "beacons_total",
            "Total number of beacons originated.", labels),
...
1. Always initialize `CounterVec` to avoid hidden metrics [link](https://prometheus.io/docs/practices/instrumentation/#avoid-missing-metrics).
