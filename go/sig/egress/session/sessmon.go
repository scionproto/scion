...
func (sm *sessMonitor) updateRemote() {
	...
	if since > tout {
		...
		sm.Debug("sessMonitor: Remote SIG timeout", "remote", sm.smRemote, "duration", since)
		...
		sm.Debug("sessMonitor: New remote", "remote", sm.smRemote)
		return
	}
	...
	if sm.smRemote.SessPath == nil {
		sm.Debug("sessMonitor: Path not available", "remote", sm.smRemote)
		sm.sess.healthy.Store(false)
		sm.smRemote.SessPath = sm.getNewPath(sm.smRemote.SessPath)
		sm.Debug("sessMonitor: New remote", "remote", sm.smRemote)
		return
	...
	if !ok {
		sm.Debug("sessMonitor: Current path was invalidated", "remote", sm.smRemote)
		sm.smRemote.SessPath = sm.getNewPath(sm.smRemote.SessPath)
...
func (sm *sessMonitor) handleRep(rpld *disp.RegPld) {
	...
	if sm.updateMsgId == rpld.Id {
		...
		if sm.smRemote.Sig.Eq(sessRemote.Sig) {
