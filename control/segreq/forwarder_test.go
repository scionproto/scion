...
func TestForwarderClassify(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Invalid Src": {
			LocalIA: nonCore111,
			Request: request{
				Src: addr.MustIAFrom(0, nonCore111.AS()),
				Dst: core110,
			},
		...
		"Core Wildcards Src & Dst": {
			LocalIA: nonCore111,
			Request: request{
				Src: addr.MustIAFrom(1, 0),
				Dst: addr.MustIAFrom(2, 0),
			},
