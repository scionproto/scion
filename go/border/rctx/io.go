...
type Sock struct {
	// Sring is a ring-buffer that's written to by writers, and read from by readers.
	Sring *sring.SRing
	...
	// Reader is an optional function that reads from Sock.Sring. It is spawned
	// in a go routine when Sock.Start() is called.
	Reader SockFunc
	// Writer is an optional function that writes to Sock.Sring. It is spawned
	// in a go routine when Sock.Start() is called.
	Writer        SockFunc
	stop          chan struct{}
...
func NewSock(sring *sring.SRing, conn conn.Conn, dir rcmn.Dir,
	ifids []common.IFIDType, locIdx int, labels prometheus.Labels, reader, writer SockFunc) *Sock {
	s := &Sock{
		Sring: sring, Conn: conn, Dir: dir, Ifids: ifids, LocIdx: locIdx, Labels: labels,
...
func (s *Sock) Stop() {
	if s.running {
		...
		s.running = false
		log.Info("Input routine stopped", "addr", s.Conn.LocalAddr())
	}
}
