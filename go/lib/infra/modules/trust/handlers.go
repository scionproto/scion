...
func (h *trcReqHandler) Handle() {
	...
	if err := messenger.SendTRC(subCtx, trcMessage, h.request.Peer, h.store.nextID()); err != nil {
...
func (h *trcPushHandler) Handle() {
	...
	result := h.store.localTRCVerify(h.request.Context(), trcObj)
	switch result {
...
func (h *chainPushHandler) Handle() {
	...
	result := h.store.localChainVerify(h.request.Context(), chain)
	switch result {
	case resultSuccessExists:
		h.log.Info("Received unsolicited TRC update from node, but already have object",
