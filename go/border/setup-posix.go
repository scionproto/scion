...
func (p posixLoc) addSock(r *Router, ctx *rctx.Ctx) error {
	...
	// XXX workaround until rinbugf metrics rework
	l := prometheus.Labels{"ringId": "loc"}
	// Setup input goroutine.
	ctx.LocSockIn = rctx.NewSock(ringbuf.New(64, nil, "in", l),
		over, rcmn.DirLocal, 0, r.posixInput, r.handleSock, PosixSock)
	ctx.LocSockOut = rctx.NewSock(ringbuf.New(64, nil, "out", l),
...
func (p posixExt) addIntf(r *Router, ctx *rctx.Ctx, intf *topology.IFInfo) error {
	...
	// XXX workaround until rinbugf metrics rework
	l := prometheus.Labels{"ringId": fmt.Sprintf("ext_%d", intf.Id)}
	// Setup input goroutine.
	ctx.ExtSockIn[intf.Id] = rctx.NewSock(ringbuf.New(64, nil, "in", l),
		c, rcmn.DirExternal, intf.Id, r.posixInput, r.handleSock, PosixSock)
	ctx.ExtSockOut[intf.Id] = rctx.NewSock(ringbuf.New(64, nil, "out", l),
