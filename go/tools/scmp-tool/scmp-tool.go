...
func SendPkts(wg *sync.WaitGroup, conn *reliable.Conn, s scmpI) {
	...
	for {
		...
		//assert.Must(s.pktSend().CmnHdr.TotalLen == uint16(pktLen), "TotalLen != pktLen")
...
func RecvPkts(wg *sync.WaitGroup, conn *reliable.Conn, s scmpI) {
	...
	for {
		...
		secs := int64(s.getTimestamp() / 1000000)
		nanos := int64((s.getTimestamp() % 1000000) * 1000)
		retTime := now.Sub(time.Unix(secs, nanos))
...
func choosePath() {
	...
	if *interactive {
		...
		if err != nil || pathIndex >= i {
			logFatal("Invalid path index", "err", err)
		}
	}
