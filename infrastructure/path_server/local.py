...
class LocalPathServer(PathServer):
    ...
    def path_resolution(self, pkt, new_request=True):
        ...
        if new_request:
            ...
            self.pending_req[(dst_ia, req.p.sibra)].append(pkt)
        else:
    ...
    def _resolve_core(self, req, up_segs, core_segs):
        ...
        for cseg in self.core_segments(**params):
            ...
            tmp_up_segs = self.up_segments(first_ia=cseg_ia, sibra=req.p.sibra)
            if tmp_up_segs:
    ...
    def _request_paths_from_core(self, req):
        ...
        """
        up_segs = self.up_segments(sibra=req.p.sibra)
        if not up_segs:
