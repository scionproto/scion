...
out_dir = '/home/tonyo/scion_ethz/scion/topology/tmp'
...
def create_next_router(generator, topo_dict):
    ...
    first_byte, mask = generator._get_subnet_params()
    if router_item:
        ...
        nr_addr = generator._increment_address(lr_addr, mask, 2)
        nr_if_addr = generator._increment_address(lr_interface_addr, mask, 2)
        ...
        ip_address_loc = ip_address('.'.join([first_byte, isd_id,
                                              ad_id, ER_RANGE]))
        ip_address_pub = generator._increment_address(ip_address_loc, mask)
        new_router = {
                    'AddrType': 'IPv4',
...
def create_new_ad(parent_ad_topo, isd_id, ad_id):
    ...
    new_topo_file = read_file(gen._path_dict(isd_id, ad_id)['topo_file_abs'])
    new_topo = json.loads(new_topo_file)

    new_topo, existing_topo = update_topos(gen, new_topo, parent_ad_topo)

    gen.write_setup_file(new_topo)
    gen.write_supervisor_config(new_topo)
    pass


# generate_config()

config_generator = ConfigGenerator(out_dir)
