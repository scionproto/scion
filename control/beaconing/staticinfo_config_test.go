...
func getTestConfigData() *beaconing.StaticInfoCfg {
	...
	return &beaconing.StaticInfoCfg{
		Latency: map[iface.IfIDType]beaconing.InterfaceLatencies{
			1: {
				Inter: w(latency_inter_1),
				Intra: map[iface.IfIDType]util.DurWrap{
		...
		LinkType: map[iface.IfIDType]beaconing.LinkType{
		...
		Geo: map[iface.IfIDType]beaconing.InterfaceGeodata{
		...
		Hops: map[iface.IfIDType]beaconing.InterfaceHops{
			1: {
				Intra: map[iface.IfIDType]uint32{
					2: hops_intra_1_2,
					3: hops_intra_1_3,
					5: hops_intra_1_5,
				},
...
func TestGenerateStaticInfo(t *testing.T) {
	...
	ifTypeNoncore := map[iface.IfIDType]topology.LinkType{
		1: topology.Child,
		2: topology.Child,
		3: topology.Parent,
		5: topology.Peer,
	...
	ifTypeCore := map[iface.IfIDType]topology.LinkType{
		1: topology.Core,
		2: topology.Child,
		3: topology.Core,
		5: topology.Core,
	...
	testCases := []struct {
		name     string
		ingress  iface.IfIDType
		egress   iface.IfIDType
		ifType   map[iface.IfIDType]topology.LinkType
