...
type sessPathPool map[paths.PathKey]*sessPath

// Return the path with the fewest failures, excluding the current path (if specified).
func (spp sessPathPool) get(currKey paths.PathKey) *sessPath {
	var sp *sessPath
...
func (spp sessPathPool) update(aps paths.AppPathSet) {
...
func newSessPath(key paths.PathKey, pathEntry *sciond.PathReplyEntry) *sessPath {
