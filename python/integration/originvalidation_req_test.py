...
class E2EClient(TestClientBase):
    ...
    def _build_pkt(self, path=None):
        ...
        extn.datahash = sha256(payload.pack())
        if misc.drkeys:
    ...
    while time.time() - start < API_TOUT:
        try:
            drkey, misc = lib_sciond.get_protocol_drkey(
                get_sciond_params(spkt, mode=OPTMode.ORIGIN_VALIDATION_ONLY, path=None),
                connector=connector)
...
def main():
    args, srcs, dsts = setup_main("OriginValidation_request")
