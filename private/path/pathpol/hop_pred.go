...
func NewHopPredicate() *HopPredicate {
	return &HopPredicate{IfIDs: make([]ifid.IfIDType, 1)}
}
...
func HopPredicateFromString(str string) (*HopPredicate, error) {
	...
	var ifIDs = make([]ifid.IfIDType, 1)
	// Parse ISD
...
func parseIfID(str string) (ifid.IfIDType, error) {
	ifID, err := strconv.ParseUint(str, 10, 64)
	if err != nil {
		return 0, err
	}
	return ifid.IfIDType(ifID), nil
}
