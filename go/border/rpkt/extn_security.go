...
func (s *rSecurityExt) Validate() (HookResult, *common.Error) {
	...
	switch {
	case sec_extn.AES_CMAC == s.SecMode && (len(s.raw) != sec_extn.AES_CMAC_TOTAL_LENGTH):
		notMatchingLen = true
	case sec_extn.HMAC_SHA256 == s.SecMode && (len(s.raw) != sec_extn.HMAC_SHA256_TOTAL_LENGTH):
		notMatchingLen = true
	case sec_extn.ED25519 == s.SecMode && (len(s.raw) != sec_extn.ED25519_TOTAL_LENGTH):
		notMatchingLen = true
	case sec_extn.GCM_AES128 == s.SecMode && (len(s.raw) != sec_extn.GCM_AES128_TOTAL_LENGTH):
		notMatchingLen = true
	case sec_extn.SCMP_AUTH_DRKEY == s.SecMode && (len(s.raw) != scmp_auth_extn.DRKEY_TOTAL_LENGTH):
		notMatchingLen = true
	case sec_extn.SCMP_AUTH_HASH_TREE == s.SecMode && (len(s.raw) != hashTreeTotalLength(hashTreeHeight(s.raw))):
		notMatchingLen = true
	}
...
func (r *rSecurityExt) GetExtn() (common.Extension, *common.Error) {
	...
	if r.SecMode == sec_extn.SCMP_AUTH_DRKEY {
	...
	} else if r.SecMode == sec_extn.SCMP_AUTH_HASH_TREE {
		...
		s, e = sec_extn.NewSecurityExtn(r.SecMode)
		if e != nil {
			return nil, e
		}
		s.(*sec_extn.SecurityExtn).UpdateMetadata(r.Metadata())
		s.(*sec_extn.SecurityExtn).UpdateAuthenticator(r.Authenticator())
	}
	return s, nil
}
...
func limitsMetadata(SecMode uint8) (int, int) {
	...
	switch SecMode {
	case sec_extn.AES_CMAC:
		l, h = limits(sec_extn.SECMODE_LENGTH, sec_extn.AES_CMAC_META_LENGTH)
	case sec_extn.HMAC_SHA256:
		l, h = limits(sec_extn.SECMODE_LENGTH, sec_extn.HMAC_SHA256_META_LENGTH)
	case sec_extn.ED25519:
		l, h = limits(sec_extn.SECMODE_LENGTH, sec_extn.ED25519_META_LENGTH)
	case sec_extn.GCM_AES128:
		l, h = limits(sec_extn.SECMODE_LENGTH, sec_extn.GCM_AES128_META_LENGTH)
	case sec_extn.SCMP_AUTH_DRKEY:
		l, h = limits(sec_extn.SECMODE_LENGTH, scmp_auth_extn.DRKEY_META_LENGTH)
	case sec_extn.SCMP_AUTH_HASH_TREE:
		l, h = limits(sec_extn.SECMODE_LENGTH, scmp_auth_extn.HASH_TREE_META_LENGTH)
	default:
...
func limitsAuthenticator(SecMode uint8, maxLen int) (int, int) {
	...
	switch SecMode {
	case sec_extn.AES_CMAC:
		l, h = limits(sec_extn.SECMODE_LENGTH+sec_extn.AES_CMAC_META_LENGTH, sec_extn.AES_CMAC_AUTH_LENGTH)
	case sec_extn.HMAC_SHA256:
		l, h = limits(sec_extn.SECMODE_LENGTH+sec_extn.HMAC_SHA256_META_LENGTH, sec_extn.HMAC_SHA256_AUTH_LENGTH)
	case sec_extn.ED25519:
		l, h = limits(sec_extn.SECMODE_LENGTH+sec_extn.ED25519_META_LENGTH, sec_extn.ED25519_AUTH_LENGTH)
	case sec_extn.GCM_AES128:
		l, h = limits(sec_extn.SECMODE_LENGTH+sec_extn.GCM_AES128_META_LENGTH, sec_extn.GCM_AES128_AUTH_LENGTH)
	case sec_extn.SCMP_AUTH_DRKEY:
		l, h = limits(sec_extn.SECMODE_LENGTH+scmp_auth_extn.DRKEY_META_LENGTH, scmp_auth_extn.DRKEY_AUTH_LENGTH)
	case sec_extn.SCMP_AUTH_HASH_TREE:
		l = sec_extn.SECMODE_LENGTH + scmp_auth_extn.HASH_TREE_META_LENGTH
