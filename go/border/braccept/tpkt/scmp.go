...
func (l *SCMP) DecodeFromBytes(data []byte, df gopacket.DecodeFeedback) error {
	...
	l.Hdr = *h
	p, err := scmp.PldFromRaw(data[scmp.HdrLen:], scmp.ClassType{l.Class, l.Type})
	if err != nil {
		return err
	}
	l.Payload = *p
...
func (l *SCMP) compareHdr(s *SCMP, lc *LayerCache) error {
	...
	// timestamp to the value of the received packet to do quick struct comparision
...
func (l *SCMP) generatePayload() error {
	// It cannot fail, as it is the packet that was sent without error
	...
	/*
		case common.L4UDP:
			qr.l4 = pkt.Layer(LayerTypeUDP).(*layers.UDP)
	*/
	case common.L4SCMP:
	...
	default:
		...
		if qr.l4 == nil {
			// Failed to decode supported trasnport layer, so retrieve payload
			qr.l4 = pkt.ApplicationLayer()
		}
	}

	if ipo, ok := l.info.(*scmp.InfoPathOffsets); ok {
		ipo.InfoF = uint16(scn.CmnHdr.CurrInfoF)
		ipo.HopF = uint16(scn.CmnHdr.CurrHopF)
	}
	pld := scmp.PldFromQuotes(scmp.ClassType{l.Class, l.Type}, l.info, l.l4Type, qr.getRaw)
	l.Payload = *pld
	return nil
}
...
/*
func NewSCMPPath(t scmp.Type, lbs []LayerBuilder) *SCMP {
	s := &SCMP{}
	s.Class = scmp.C_Path
	s.Type = t
	s.lbs = lbs
	...
	case scmp.T_P_BadMac:
	case scmp.T_P_ExpiredHopF:
	case scmp.T_P_BadIF:
	case scmp.T_P_RevokedIF:
	case scmp.T_P_NonRoutingHopF:
	case scmp.T_P_DeliveryNonLocal:
	case scmp.T_P_BadSegment:
	case scmp.T_P_BadInfoField:
	case scmp.T_P_BadHopField:
	}
	return s
}
*/
func NewInfoPathOffsets(infoF, hopF int, ifid common.IFIDType, in bool) *scmp.InfoPathOffsets {
