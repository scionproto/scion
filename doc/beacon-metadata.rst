...
This is the generalized protobuf-like description for a per-link metadata item
of type ``T`` (buto protobuf does not actually have a templates).
...
A Note is simply a bit of plaintext. Use cases of such information
include:
...
for the PCBs it sends out from a JSON cofiguration file, ``staticInfoConfig.json``.
...
The config file for this AS could then look like this:
