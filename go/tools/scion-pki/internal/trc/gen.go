...
func newTrc(isd addr.ISD, iconf *conf.Isd, path string) (*trc.TRC, error) {
	...
	for _, cia := range iconf.Trc.CoreIAs {
		...
		if a.SignAlgorithm.OnlineKeyAlg != "" {
			as.OnlineKeyAlg = a.SignAlgorithm.OnlineKeyAlg
		}
		as.OfflineKeyAlg = crypto.Ed25519
		if a.SignAlgorithm.OfflineKeyAlg != "" {
			as.OfflineKeyAlg = a.SignAlgorithm.OfflineKeyAlg
		}
