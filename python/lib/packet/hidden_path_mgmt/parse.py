...
from lib.packet.hidden_path_mgmt.cfg_recs import (
    HiddenPathConfigRecordsReg,
    HiddenPathConfigRecordsReply,
)
from lib.packet.hidden_path_mgmt.cfg_req import HiddenPathConfigReq
from lib.packet.hidden_path_mgmt.seg_recs import (
    HiddenPathRecordsReg,
    HiddenPathRecordsReply,
)
from lib.packet.hidden_path_mgmt.seg_req import HiddenPathSegmentReq


def parse_hiddenpathmgmt_payload(wrapper):  # pragma: no cover
    type_ = wrapper.which()
    ...
    for cls_ in (
        ...
        if cls_.PAYLOAD_TYPE == type_:
            return cls_(getattr(wrapper, type_), timestamp, signature)
