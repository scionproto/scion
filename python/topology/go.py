...
class GoGenerator(object):
    ...
    def _build_br_conf(self, topo_id, ia, base, name, v):
        ...
        raw_entry = {
            ...
            'metrics': {
                'prometheus':
                    prom_addr(v['internal_addr'], DEFAULT_BR_PROM_PORT),
            },
    ...
    def _build_control_service_conf(self, topo_id, ia, base, name, infra_elem,
        ...
        raw_entry = {
            ...
            'tracing':
                self._tracing_entry(),
            'metrics':
                self._metrics_entry(infra_elem, CS_PROM_PORT),
            'quic':
                self._quic_conf_entry(CS_QUIC_PORT, self.args.svcfrac,
                                      infra_elem),
            'features':
                translate_features(self.args.features),
        }
        if ca:
    ...
    def generate_co(self):
        ...
        for topo_id, topo in self.args.topo_dicts.items():
            for elem_id, elem in topo.get("colibri_service", {}).items():
                ...
                if elem_id.endswith("-1"):
                    base = topo_id.base_dir(self.args.output_dir)
                    co_conf = self._build_co_conf(topo_id, topo["isd_as"], base,
                                                  elem_id, elem)
                    write_file(os.path.join(base, elem_id, CO_CONFIG_NAME),
                               toml.dumps(co_conf))
    ...
    def _build_co_conf(self, topo_id, ia, base, name, infra_elem):
        ...
        raw_entry = {
            ...
            'tracing':
                self._tracing_entry(),
            'metrics':
                self._metrics_entry(infra_elem, CO_PROM_PORT),
            'quic':
                self._quic_conf_entry(CO_QUIC_PORT, self.args.svcfrac,
                                      infra_elem),
            'features':
                translate_features(self.args.features),
        }
    ...
    def _build_co_traffic_matrix(self, ia):
        ...
        if_ids = {
            iface for br in topo['border_routers'].values()
        ...
        if this_as['Core']:
            ...
            for dst_ia, topo in self.args.topo_dicts.items():
                if dst_ia != ia and dst_ia._isd == ia._isd and topo['Core']:
                    ...
                    rsvps['Up-%s' % dst_ia] = self._build_co_reservation(
                        dst_ia, 'Up')
    ...
    def generate_disp(self):
        if self.args.docker:
            ...
            write_file(config_file_path,
                       toml.dumps(self._build_disp_conf("dispatcher")))
    ...
    def _gen_disp_docker(self):
        for topo_id, topo in self.args.topo_dicts.items():
            ...
            for k in elem_ids:
                ...
                write_file(os.path.join(base, f'{disp_id}.toml'),
