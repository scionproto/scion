...
class ISD_AS(Serializable):
    """
    Class for representing ISD-AS pair. The underyling type is a 64bit int; ISD is represented by
    the top 16 bits (though the top 4 bits are currently reserved), and AS by the lower 48 bits.
    """
    NAME = "ISD_AS"
    LEN = 8
    ISD_BITS = 16
    ...
    def _parse_bytes(self, raw):
        """
        :param bytes raw: a byte string containing a 64-bit integer.
        """
    ...
    def _parse_int(self, raw):
        """
        :param int raw: a 64-bit integer
        """
