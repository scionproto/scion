...
uint16_t scion_udp_checksum(SCIONCommonHeader *sch)
{
  ...
  payload_len -= sizeof(struct udp_hdr); // Length in UDP header includes header size, so subtract it.
...
void update_scion_udp_checksum(SCIONCommonHeader *sch){
  ...
  scion_udp_hdr->dgram_cksum = 0;
  scion_udp_hdr->dgram_cksum = htons(scion_udp_checksum(sch));
