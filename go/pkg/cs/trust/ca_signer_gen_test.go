...
import (
	...
	mock_cstrust "github.com/scionproto/scion/go/pkg/cs/trust/mock_trust"
...
func TestChachingPolicyGenGenerate(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		PolicyGen    func(mctrl *gomock.Controller) cstrust.PolicyGen
		...
		"valid": {
			Interval: time.Hour,
			PolicyGen: func(mctrl *gomock.Controller) cstrust.PolicyGen {
				gen := mock_cstrust.NewMockPolicyGen(mctrl)
...
func TestLoadingPolicyGenGenerate(t *testing.T) {
	...
	testCases := map[string]struct {
		CertProvider func(*testing.T, *gomock.Controller) cstrust.CACertProvider
		KeyRing      func(*testing.T, *gomock.Controller) trust.KeyRing
		ErrAssertion assert.ErrorAssertionFunc
	}{
		"valid": {
			CertProvider: func(t *testing.T, mctrl *gomock.Controller) cstrust.CACertProvider {
				p := mock_cstrust.NewMockCACertProvider(mctrl)
		...
		"err key ring": {
			CertProvider: func(t *testing.T, mctrl *gomock.Controller) cstrust.CACertProvider {
				return mock_cstrust.NewMockCACertProvider(mctrl)
			},
			KeyRing: func(t *testing.T, mctrl *gomock.Controller) trust.KeyRing {
				k := mock_trust.NewMockKeyRing(mctrl)
...
func loadKey(t *testing.T, file string) crypto.Signer {
	...
	b, _ := pem.Decode(raw)
	require.NotNil(t, b)
	s, err := x509.ParsePKCS8PrivateKey(b.Bytes)
	require.NoError(t, err)
	return s.(crypto.Signer)
}
