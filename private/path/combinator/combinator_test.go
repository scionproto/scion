...
func TestNonWorkingPeering(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Name: "non working peering",
			// Empty file so it's easier to see the output
			FileName: "empty.txt",
...
func TestNonWorkingPeeringReverse(t *testing.T) {
	ctrl := gomock.NewController(t)
	g := graph.NewFromDescription(ctrl, graph.BigGraphDescription)
...
func TestNonWorkingPeeringA(t *testing.T) {
	ctrl := gomock.NewController(t)
	g := graph.NewFromDescription(ctrl, graph.BigGraphDescription)
