...
func (t Target) String() string {
	return fmt.Sprintf("<%s, %s, %d, %d, %d>", t.Quality, t.Direction, t.Uniquifier, t.ISD, t.AS)
}
...
func (t Target) getMetric(ctx context.Context, bcn beacon.Beacon) float64 {
	...
	for _, entry := range bcn.Segment.ASEntries {
		...
		new_val := t.extractMetricFromASEntry(ctx, &entry)
...
func (t Target) extractMetricInterFrom(ctx context.Context, ase *seg.ASEntry, ifid uint16) float64 {
...
func (t Target) extractMetricIntraTo(ctx context.Context, ase *seg.ASEntry, ifidTo uint16) float64 {
	staticInfo := ase.Extensions.StaticInfo
...
func (t Target) extractMetricFromASEntry(ctx context.Context, ase *seg.ASEntry) float64 {
	staticInfo := ase.Extensions.StaticInfo
	...
	interLinkMetric := t.extractMetricInterFrom(ctx, ase, egIfId)
	intraLinkMetric := t.extractMetricIntraTo(ctx, ase, ingIfId)
