...
func (c *SCIONConn) createUDPPacket(b []byte, raddr *SCIONAppAddr,
	...
	addrHdrLen := raddr.host.Size() + c.laddr.host.Size() + raddr.ia.SizeOf() +
		c.laddr.ia.SizeOf()
	// Pad to multiple of LineLen
	if addrHdrLen > common.MaxAddrHdrLen {
		return nil, common.NewError("Address header length exceeds maximum value",
			"len", addrHdrLen, "max", common.MaxAddrHdrLen)
	}
	addrHdrLen += (common.MaxAddrHdrLen - addrHdrLen) % common.LineLen
	// We do not include the L4 header size in the total header size
	hdrLen := cmnHdrLen + addrHdrLen + len(path.Path.FwdPath)
