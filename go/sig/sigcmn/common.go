...
func Init(ia *addr.ISD_AS, ip net.IP) error {
	...
	if *CtrlPort < 1 || *CtrlPort > maxPort {
		return common.NewCError("Invalid local ctrl port",
			"min", 1, "max", maxPort, "actual", *CtrlPort)
	}
	if *EncapPort < 1 || *EncapPort > maxPort {
		return common.NewCError("Invalid local encap port",
			"min", 1, "max", maxPort, "actual", *EncapPort)
	}
	return nil
}
