...
func ProcessSelector(selector string) (map[addr.ISD][]addr.IA, error) {
	...
	if len(isdDirs) == 0 {
		return nil, common.NewBasicError("No ISD directories found for", nil, "selector", selector)
	}
	...
	for _, dir := range isdDirs {
		...
		if len(dirs) == 0 {
			return nil, common.NewBasicError("No AS directories found for", nil, "selector", selector)
		}
