...
type SCMPAuthExtn struct {
	pkt_sec_extn.SCIONPacketSecurityExtn
}
...
const (
	...
	DRKEY_META_LENGTH  = DIRECTION_LENGTH + 3 // padding
	DRKEY_AUTH_LENGTH  = MAC_LENGTH
	DRKEY_TOTAL_LENGTH = pkt_sec_extn.SECMODE_LENGTH + DRKEY_META_LENGTH + DRKEY_AUTH_LENGTH

	HASH_TREE_META_LENGTH = HEIGHT_LENGTH + ORDER_LENGTH
)

const (
	AS_TO_AS              byte = iota // Signed with S -> D
...
func NewSCMPDRKeyAuthExtn(SecMode uint8) (*SCMPAuthExtn, *common.Error) {
	if SecMode != pkt_sec_extn.SCMP_AUTH_DRKEY {
		return nil, common.NewError("Invalid secmode code.", "SecMode", SecMode)
	}
	s := &SCMPAuthExtn{SCIONPacketSecurityExtn: pkt_sec_extn.SCIONPacketSecurityExtn{SecMode: SecMode}}
...
func (s SCMPAuthExtn) UpdateDirection(dir byte) *common.Error {
...
func (s SCMPAuthExtn) Direction(dir byte) byte {
	if s.SecMode != pkt_sec_extn.SCMP_AUTH_DRKEY {
		return AS_TO_AS
	}
	return s.Metadata[0]
