...
type requestI interface {
	...
	// getCompletionChan returns the channel the goroutine that submitted the
	// request is waiting on.
	getCompletionChan() chan<- error
}
...
func (req trcRequest) resolve(ctx context.Context, msger infra.Messenger, db *trustdb.DB) error {
	...
	trcMessage, err := msger.GetTRC(ctx, trcReqMsg, req.hint, req.id)
	if err != nil {
		return err
	}
...
func (req chainRequest) resolve(ctx context.Context, msger infra.Messenger, db *trustdb.DB) error {
	...
	chainMessage, err := msger.GetCertChain(ctx, chainReqMsg, req.hint, req.id)
	if err != nil {
...
func (req chainRequest) getCompletionChan() chan<- error {
	return req.errChan
}
