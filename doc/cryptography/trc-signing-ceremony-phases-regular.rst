.. _trc-signing-ceremony-phases-regular:

*********************************************
TRC Signing Ceremony Phases - Regular Update
*********************************************
...
- :ref:`ceremony-administrator-role-regular-update`
- :ref:`voting-as-representative-role-regular-update`
- :ref:`witness-role-regular-update`
...
to include new regular voting certificates that were previously not part of the
predecessor TRC. Also for the purpose of this document, ``bern`` is assumed to be a
Certificate Authority for the ISD.

.. _ceremony-administrator-role-regular-update:
...
The *ceremony administrator* clarifies that only the certificates that
are new or changed compared to the predecessor TRC should be exchanged.
For a regular update, the *CP sensitive voting certificates* must not change
and therefore must not be exchanged. This means the *voting representatives*
only need to share their *CP regular voting certificate*, and, if applicable,
their *CP root certificate*.

.. note::

   The *CP sensitive voting certificates* are not exchanged in a regular TRC
   update. The ceremony administrator uses the sensitive voting certificates
   from the predecessor TRC when creating the TRC payload.
...
.. tab-set::
   ...
   .. tab-item:: scion-pki
      ...
      .. literalinclude:: trc_ceremony_regular.sh
...
``{{.Votes}}`` variable below. Votes contains a sequence of indices of the
regular voting certificates in the predecessor TRC. To find the indices, you
can use the ``scion-pki trc inspect`` command.
...
previously not included regular voting certificate must show
proof-of-possession, i.e., show that they have access to the private key listed
in these fresh certificates. This is done by signing the TRC with the
respective private key. The phase concludes after all *voting representatives*
have cast their votes, the applicable parties have shown proof-of-possession,
and copied the resulting signatures onto the *USB flash drive*.
...
- ``/bern/isd.regular.vote.trc``
- ``/geneva/isd.regular.vote.trc``
- ``/zürich/isd.regular.vote.trc``

The proof-of-possessions for the freshly included regular voting certificates
should be available at the following locations on the *USB flash drive*:
...
.. _voting-as-representative-role-regular-update:
...
To verify the resulting TRC with openssl, regular voting certificates
of the predecessor TRC must be available as PEM bundle.

To cast a vote, the *voting representative* needs access to the regular
...
``$KEYDIR/$PREDID/regular-voting.key`` and the certificate at
...
Phase 1 consists of sharing the new regular voting and root certificates that
...
Copy the *CP Regular Voting Certificate* onto the *USB Flash Drive* into the
newly created folder. The regular voting certificate should be named
``regular-voting.crt``.

.. Warning::

   Note that only the certificate must be shared during this step, not the
   private key. Copying a private key by mistake invalidates the security of
   the ceremony.

.. note::

   The *CP Sensitive Voting Certificate* must not be shared and must not be
   included in the folder. In a regular TRC update, the sensitive voting
   certificates remain unchanged from the predecessor TRC.
...
Furthermore, check that the regular voting certificate that is on the *USB flash
drive* does not differ from your own. For example, for identifier ``bern``, the
command that should be executed in the same folder as above is:
...
This phase consists of casting a vote on the TRC payload using the *regular
voting certificate* from the predecessor TRC. If the *regular voting
certificate* is new (not present in the predecessor TRC), a proof-of-possession
signature must also be computed using the new certificate.
...
If the *regular voting certificate* is new, compute the proof-of-possession
signature by running:
...
.. tab-set::
   ...
   .. tab-item:: scion-pki
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE sign_regular_payload_scion_pki START
         :end-before: LITERALINCLUDE sign_regular_payload_scion_pki END
         :dedent: 4
   ...
   .. tab-item:: openssl
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE sign_regular_payload START
         :end-before: LITERALINCLUDE sign_regular_payload END
         :dedent: 4
...
.. tab-set::
   ...
   .. tab-item:: scion-pki
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE regular_vote_scion_pki START
         :end-before: LITERALINCLUDE regular_vote_scion_pki END
         :dedent: 4
   ...
   .. tab-item:: openssl
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE regular_vote START
         :end-before: LITERALINCLUDE regular_vote END
         :dedent: 4
...
To sanity check that the proof-of-possession signature was created correctly, run:
...
.. tab-set::
   ...
   .. tab-item:: scion-pki
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE check_regular_signed_payload_scion_pki START
         :end-before: LITERALINCLUDE check_regular_signed_payload_scion_pki END
         :dedent: 4
   ...
   .. tab-item:: openssl
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE check_regular_signed_payload START
         :end-before: LITERALINCLUDE check_regular_signed_payload END
         :dedent: 4
...
.. tab-set::
   ...
   .. tab-item:: scion-pki
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE check_regular_vote_scion_pki START
         :end-before: LITERALINCLUDE check_regular_vote_scion_pki END
         :dedent: 4
   ...
   .. tab-item:: openssl
      ...
      .. literalinclude:: crypto_lib.sh
         :start-after: LITERALINCLUDE check_regular_vote START
         :end-before: LITERALINCLUDE check_regular_vote END
         :dedent: 4

Connect the *USB flash drive* to the *device*, and copy ``$TRCID.regular.trc``
(if a new regular voting certificate was included) and ``$TRCID.regular.vote.trc``
to the folder named after your identifier.
...
signature for each regular voting certificate that was previously not part of
the predecessor TRC. If a signature is missing, then **Phase 3** and **Phase 4**
need to be repeated.
