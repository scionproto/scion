...
func TestHVFVerification(t *testing.T) {
	...
	err = libepic.VerifyHVF(authPenultimate, epicpath, s, timestamp, false)
	assert.NoError(t, err)
	err = libepic.VerifyHVF(randBytes(16), epicpath, s, timestamp, false)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authPenultimate, nil, s, timestamp, false)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authPenultimate, epicpath, nil, timestamp, false)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authPenultimate, epicpath, s, timestamp-10, false)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authPenultimate, epicpath, s, timestamp, true)
	assert.Error(t, err)
	...
	err = libepic.VerifyHVF(authLast, epicpath, s, timestamp, true)
	assert.NoError(t, err)
	err = libepic.VerifyHVF(randBytes(16), epicpath, s, timestamp, true)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authLast, nil, s, timestamp, true)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authLast, epicpath, nil, timestamp, true)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authLast, epicpath, s, timestamp+10, true)
	assert.Error(t, err)
	err = libepic.VerifyHVF(authLast, epicpath, s, timestamp, false)
	assert.Error(t, err)
}
