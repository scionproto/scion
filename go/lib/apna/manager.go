...
func handleRequest(buf []byte, addr *net.UDPAddr) (ephID *EphID) {
	...
	case 0x02:
		...
		log.Info("received msg before calculation: ", buf[1:17])
		result := verifyMac(buf[1:13], buf[13:])
		log.Info("mac verification:", result)
		ephID = decryptEphID(buf[1:5], buf[5:13])
	}
	return
}
...
func handleConnection(serverConn *net.UDPConn, quit chan struct{}) {
	for {
		...
		if err != nil {
			...
			ephID := handleRequest(buf[0:n], addr)
			iv, finalEphID := encryptEphID(ephID)
			mac := computeMac(iv, finalEphID)
			response := append(iv, finalEphID...)
