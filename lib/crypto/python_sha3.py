...
class Keccak:
  ...
  def update(self, arg):
    ...
    self.buffered_data += encode(bytes(arg, 'utf-8'), 'hex').decode('utf-8')
  ...
  def digest(self):
    ...
    M = _build_message_pair(self.buffered_data)
    ...
    self.last_digest = decode(Z[:2 * self.n // 8], 'hex')
    return self.last_digest
