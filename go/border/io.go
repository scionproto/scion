...
// posixInput returns a socket function which handles input from a
// posix socket for a specific router.
func posixInput(r *Router) rctx.SockFunc {
	...
	Top:
		for {
			...
			pktsRead, err := r.posixInputRead(msgs[:toRead], readMetas[:toRead], s.Conn)
			if err != nil {
				inputReadErrs.Inc()
...
// posixOutput returns a socket function which handles output to a
// posix socket for a specific router.
func posixOutput(r *Router) rctx.SockFunc {
	return func(s *rctx.Sock, _, stopped chan struct{}) {
		defer log.LogPanicAndExit()
		defer close(stopped)
