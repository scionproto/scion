...
func (r *Router) writeLocalOutput(out *net.UDPConn, labels prometheus.Labels, p *rpkt.RPkt) {
	if len(p.Egress) == 0 {
...
func (r *Router) writeIntfOutput(out *net.UDPConn, labels prometheus.Labels, p *rpkt.RPkt) {
	if len(p.Egress) == 0 {
		p.Error("Destination not specified")
		return
	}
