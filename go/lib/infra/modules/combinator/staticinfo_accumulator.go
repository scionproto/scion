...
func combineSegs(ases *asEntryList) *PathMetadata {
	allEntries := append(append(append([]*seg.ASEntry(nil), ases.Ups...), ases.Cores...), ases.Downs...)
	...
	coreStart := -1
	if len(ases.Cores) > 0 {
		coreStart = len(ases.Ups)
	}
	downStart := -1
	if len(ases.Downs) > 0 {
		downStart = coreStart + len(ases.Cores)
	}

	asEntry := allEntries[0]
	s := asEntry.Exts.StaticInfo
	// For the first AS on the path, only extract
	// the note and the geodata, since all other data
	// is not available as part of the saved
	...
	for idx := 0; idx < len(allEntries); idx++ {
		...
		case 0 == idx:
			res.Geo[asEntry.IA()] = getGeo(asEntry)
			res.Notes[asEntry.IA()] = ASnote{Note: s.Note}
		case ((len(ases.Ups)-1) == idx) && (-1 == coreStart) && (-1 == downStart):
				extractSingleSegmentFinalASData(res, asEntry)
		case  (coreStart-1) == idx:
			continue
		case (coreStart == idx) && (coreStart != 0):
			// crossover to core segment
			extractUpOverdata(res, allEntries[idx-1], asEntry)
		case (coreStart == idx) && (coreStart == 0):
			// first AS in the path
			res.Geo[asEntry.IA()] = getGeo(asEntry)
			res.Notes[asEntry.IA()] = ASnote{Note: s.Note}
		case ((len(ases.Ups)+len(ases.Cores)-1)==idx) && (downStart==-1):
			extractSingleSegmentFinalASData(res, asEntry)
		case (downStart-1) == idx:
			continue
		case downStart == idx && ases.DownPeer == 0:
			// crossover to down segment
			extractCoreOverdata(res, allEntries[idx-1], asEntry)
		case downStart == idx && ases.DownPeer != 0:
			// peering from up segment
			prevPeerEntry := allEntries[idx-1].HopEntries[ases.UpPeer]
			pe, _ := prevPeerEntry.HopField()
			extractPeerdata(res, asEntry, pe.ConsIngress, false)

			peerEntry := asEntry.HopEntries[ases.DownPeer]
			pe, _ = peerEntry.HopField()
			extractPeerdata(res, asEntry, pe.ConsIngress, true)
		default:
			// simple continuation on the segment
			extractNormaldata(res, asEntry)
		}
	}
	// Collect the final AS on the path:
	asEntry = allEntries[len(allEntries)-1]
	extractSingleSegmentFinalASData(res, asEntry)
	return res
}
