...
func addPosixLocal(r *Router, ctx *rctx.Ctx, idx int, ba *topology.AddrInfo,
	...
	ctx.LocSockIn[idx] = rctx.NewSock(newRpktSRing(1024, "locIn", labels), over, rcmn.DirLocal,
		ifids, idx, labels, r.posixInput, r.handleSock)
	ctx.LocSockOut[idx] = rctx.NewSock(newRpktSRing(1024, "locOut", labels), over, rcmn.DirLocal,
...
func addPosixIntf(r *Router, ctx *rctx.Ctx, intf *netconf.Interface,
	...
	// Setup input goroutine.
	ctx.ExtSockIn[intf.Id] = rctx.NewSock(newRpktSRing(1024, "extIn", labels),
		c, rcmn.DirExternal, ifids, -1, labels, r.posixInput, r.handleSock)
	ctx.ExtSockOut[intf.Id] = rctx.NewSock(newRpktSRing(1024, "extOut", labels),
		c, rcmn.DirExternal, ifids, -1, labels, nil, r.posixOutput)
	log.Debug("Set up new external socket.", "intf", intf)
	return nil
}

func newRpktSRing(size int, desc string, labels prometheus.Labels) *sring.SRing {
	return sring.New(size, func() interface{} {
		return rpkt.NewRtrPkt()
	}, desc, labels)
}

func newEgressRpktSRing(size int, desc string, labels prometheus.Labels) *sring.SRing {
	return sring.New(size, func() interface{} {
