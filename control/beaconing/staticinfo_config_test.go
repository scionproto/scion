...
func getTestConfigData() *beaconing.StaticInfoCfg {
	...
	return &beaconing.StaticInfoCfg{
		Latency: map[ifid.IfIDType]beaconing.InterfaceLatencies{
			1: {
				Inter: w(latency_inter_1),
				Intra: map[ifid.IfIDType]util.DurWrap{
		...
		Bandwidth: map[ifid.IfIDType]beaconing.InterfaceBandwidths{
			1: {
		...
		LinkType: map[ifid.IfIDType]beaconing.LinkType{
		...
		Geo: map[ifid.IfIDType]beaconing.InterfaceGeodata{
		...
		Hops: map[ifid.IfIDType]beaconing.InterfaceHops{
			1: {
				Intra: map[ifid.IfIDType]uint32{
					2: hops_intra_1_2,
					3: hops_intra_1_3,
					5: hops_intra_1_5,
				},
...
func TestGenerateStaticInfo(t *testing.T) {
	...
	ifTypeNoncore := map[ifid.IfIDType]topology.LinkType{
		1: topology.Child,
		2: topology.Child,
		3: topology.Parent,
		5: topology.Peer,
	...
	ifTypeCore := map[ifid.IfIDType]topology.LinkType{
		1: topology.Core,
		2: topology.Child,
		3: topology.Core,
		5: topology.Core,
	...
	testCases := []struct {
		...
		ifType   map[ifid.IfIDType]topology.LinkType
