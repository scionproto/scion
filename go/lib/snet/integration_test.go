...
import (
	...
	_ "github.com/scionproto/scion/go/lib/scrypto" // Make sure math/rand is seeeded
...
func ClientServer(haveSciond bool, idx int, tc TestCase) {
	Convey(fmt.Sprintf("Test %v: (%v-%v,%v):%v <-> (%v-%v,%v):%v", idx, tc.srcIA.I,
		...
		sconn, err := serverNet.ListenSCION("udp4", serverAddr, 0)
		...
		cconn, err := clientNet.DialSCION("udp4", clientAddr, serverAddr, 0)
