...
Calculations at Border Router
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The MAC calculations in border routers works as follows for packets in down (``ConsDir == 1``) and up (``ConsDir == 0``) direction:

Down
    #. Calculate :math:`\sigma_i` (see above) where :math:`\beta_i` is the
       current value of ``SegID``. 
    #. Calculate :math:`\beta_{i+1} = \beta_i \oplus
       \sigma_i[:2]` and write it to the ``SegID`` field.

Up
    #. Calculate :math:`\beta_{i} = \beta_{i+1} \oplus \sigma'_i[:2]`, 
       where :math:`\sigma'_i` is the current value of the `MAC` field.
    #. Calculate :math:`\sigma_i` (see above). #. Write :math:`beta_{i}` to the 
       ``SegID`` field

Peer
    - No changes to the calculations required in **down** direction.
    - In **up** direction, calculate instead :math:`\beta_i = \beta_{i+1}`
