...
go_lint() {
    ...
    out=$($GOSDK/gofmt -d -s $LOCAL_DIRS); if [ -n "$out" ]; then echo "$out"; ret=1; fi
    echo "======> linelen (lll)"
    out=$(find go -type f -iname '*.go' -a '!' '(' -ipath 'go/lib/pathpol/sequence/*' ')' | $TMPDIR/lll -w 4 -l 100 --files -e '`comment:"|`ini:"|https?:'); if [ -n "$out" ]; then echo "$out"; ret=1; fi
