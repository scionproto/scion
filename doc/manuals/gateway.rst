...
Once both SIGs are running, IP traffic matching the configured prefixes is automatically tunneled over SCION. Each SIG independently discovers remote SIGs via :ref:`SIG Discovery <sig-discovery>`, fetches their prefix announcements via :ref:`SGRP <sgrp>`, and creates tunneling sessions. Note that path policies for SIGs are not configurable now.
