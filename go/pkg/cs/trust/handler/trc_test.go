...
func TestTRCReqHandle(t *testing.T) {
	ia110 := xtest.MustParseIA("1-ff00:0:110")
	internal := serrors.New("internal")
	mockTRC := cppki.SignedTRC{Raw: []byte("mock TRC")}
	...
	tests := map[string]struct {
		...
		"no messenger": {
			Request: func(ctrl *gomock.Controller) *infra.Request {
				return infra.NewRequest(context.Background(), &cert_mgmt.TRCReq{}, nil, nil, 0)
			},
			Provider: func(ctrl *gomock.Controller) trust.Provider {
				return mock_trust.NewMockProvider(ctrl)
			},
		...
		"trustengine error": {
			...
			Provider: func(ctrl *gomock.Controller) trust.Provider {
				...
				p.EXPECT().GetSignedTRC(gomock.Any(),
					cppki.TRCID{
						ISD:    1,
						Base:   1,
						Serial: 3,
				...
				).Return(cppki.SignedTRC{}, internal)
				return p
			},
		...
		"send error": {
			Request: func(ctrl *gomock.Controller) *infra.Request {
				rw := mock_infra.NewMockResponseWriter(ctrl)
				pld := &cert_mgmt.TRC{RawTRC: mockTRC.Raw}
	...
	for n, tc := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			defer ctrl.Finish()
			h := handler.TRCReq{Provider: test.Provider(ctrl), IA: ia110}
